...
void SerializeTrajectoryNodes(
  ...
  for (const auto& node_id_data : trajectory_nodes) {
    SerializedData proto;
    auto* const node_proto = proto.mutable_node();　　　　　　　　　　　　　　　　　　　//从该函数的实现上来看，该函数返回指向该字段的一个指针。
                                                                                     //同时将该字段置为被设置状态。
...
void SerializeTrajectoryData(
    const std::map<int, PoseGraphInterface::TrajectoryData>& all_trajectory_data,
    ProtoStreamWriterInterface* const writer) 
{
...
// 调用了以上所有传感器写入器，主要写入pose_graph 反序列器内
void WritePbStream(
    ...
    bool include_unfinished_submaps) 
{
  ...
  SerializeSubmaps(           pose_graph.GetAllSubmapData(), 
                              include_unfinished_submaps,
                              writer);

  SerializeTrajectoryNodes(   pose_graph.GetTrajectoryNodes(),
                              writer);

  SerializeTrajectoryData(    pose_graph.GetTrajectoryData(),
                              writer);

  SerializeImuData(           pose_graph.GetImuData(), 
                              writer);

  SerializeOdometryData(      pose_graph.GetOdometryData(), 
                              writer);

  SerializeFixedFramePoseData(pose_graph.GetFixedFramePoseData(), 
                              writer);

  SerializeLandmarkNodes(     pose_graph.GetLandmarkNodes(), 
                              writer);
}
